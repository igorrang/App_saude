...
import { Box, View, HStack, Icon, Heading, Text, Button, Center, Image, ScrollView, Pressable, IconButton } from 'native-base'
...
export default function Registro({ navigation }) {
    ...
    const verificaMatricula = () => {
        ...
        apiFetch(`https://sechml.procergs.com.br/sbi-services/api/ipespa/grupo-familiar/${matricula}`)
            .then((res) => {
                return res.data
            }).then((data) => {
                ...
                if (teste == 'Matrícula não encontrada.') {
                    ...
                    //  console.log(teste)
                    const usercredentials = auth().createUserWithEmailAndPassword(email, password).then((usercredentials) => {
                        if (usercredentials) {
    ...
    return (
        ...
        <View style={{ flex: 1 }}  >
            ...
            <LinearGradient flex={1} colors={['#fff', '#f1ffe8', '#D5FED4']} locations={[0, 0.9, 1]} >
                <ScrollView contentContainerStyle={{ flexGrow: 1, }} flex={0.2} >

                <View flex={1} mx='4'>
                    <Box flex={0.45} justifyContent='flex-start' alignItems='center'>
                        
                        <IconButton onPress={() => navigation.navigate('Login')} mt={2} _pressed={{ backgroundColor: 'gray.200' }} borderRadius={'full'} position={'absolute'} alignSelf={'flex-start'} 
                        icon={<Icon as={MaterialIcons} name="arrow-back" size={'lg'} color='gray.300' />}/>
                        <Image
                            source={require('../assets/ipe_logo.png')}
                            style={{ width: 300, height: 250 }}
                            alt="Alternate Text" />
                    </Box>
                    <Box paddingBottom={10} flex={0.55} justifyContent='flex-start' alignItems='center'>
                        <Heading mb={2} color="orange.500" size="lg">Cadastre-se</Heading>
                        <Text mb={6} color="#005B36">Por favor, preencha o formulário abaixo:</Text>
                        ...
                        <InputRegistro placeholder="Digite sua senha"
                        ...
                        <InputRegistro placeholder="Confirme sua senha"
                            type='password'
                        ...
                        <InputRegistro
                            ...
                            maxLength={13} 
                            InputLeftElement={<Icon as={<MaterialIcons name="person" />} ml={3} mr={1.5} size={6} color="gray.300" />}/>
                        ...
                        <View mt={-1} width={'100%'} flexDir={'column'}>
                            ...
                            <CheckBox size={18} uncheckedColor={'#005B36'} containerStyle={{ backgroundColor: 'transparent', padding: -5, marginRight: -2, marginBottom: 2, marginLeft: -1 }}
                                    checkedColor={'#005B36'} checked={isChecked} onPress={() => setIsChecked(!isChecked)} />
