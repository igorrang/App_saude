...
export default function Perfil() {
  ...
  const { logout, userNome, userMatricula } = useContext(AuthContext);
  ...
  return (
    <Box>
      <View backgroundColor='orange.500' height={55} alignItems='center' paddingY={2} flexDirection='row' borderBottomRadius={15} justifyContent='center'>
        <Text fontSize={20} color='white'>Perfil</Text>
      </View>
      ...
      >
        ...
        >
          ...
          <Box flexDirection="column">
            <Text fontSize={18} color="gray.600" fontWeight="bold">{nome}</Text>
        ...
        <Box flexDirection="column" alignItems='center' bgColor='white' shadow={5} m={1} borderRadius={25} >
          <FlatList
            data={Users}
            vertical
          ...
          />
          <Button
            m={2} rounded={100} px={4} bg="green.600" leftIcon={<Icon as={Ionicons} name="create-outline" size={18} />} width='35%' shadow={2} alignSelf="flex-end"
        ...
        <Box mt={5} mb={5} flexDirection="column" alignItems='center' bgColor='white' shadow={5} m={1} borderRadius={25} >
          <FlatList
            data={Users}
            vertical
            keyExtractor={(item, index) => index.toString()}
        ...
        <Button w='97%' alignSelf='center' mt='3' bg="green.600" mb='20' borderRadius={15} shadow={2} _pressed={{ backgroundColor: 'green.600', opacity: 0.5 }} onPress={() => logout()} leftIcon={<Icon as={Ionicons} name="exit" size="lg" />}>
          Sair
        </Button>
      </ScrollView>
